...
const handleView = () => {
if (!user) navigate("/login", { state: { from: /food/${_id} } });
else navigate(/food/${_id});
};

return (
<div className="card bg-base-200 hover:bg-base-300 client-smooth shadow-xl group">
<figure className="overflow-hidden h-56">
<img src={foodImage} alt={foodName} className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-300" />
</figure>
...
<div className="col-span-2">Expires: {new Date(expireDate).toLocaleDateString()}</div>
</div>
<div className="card-actions mt-2">
<button onClick={handleView} className="btn btn-primary w-full">View Details</button>
</div>
</div>
</div>
);
